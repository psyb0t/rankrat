...
Connection-level provider failures now explain themselves. When a fixed-origin
host refuses to connect because it resolves to a DNS sinkhole (`0.0.0.0` /
`127.0.0.1` — the signature of a blocker like AdGuard or Pi-hole), the error
gains a curated `detail` naming the host and the address it resolved to, instead
of only `provider operation failed`. This surfaces the common case where a DNS
blocker filters the Google Analytics domains and just the GA4 tools go dark while
Search Console keeps working.

- The `detail` is generated by Rankrat from the fixed host and its resolved
  address alone — never provider prose, credentials, or upstream payloads — so
  the redaction guarantee is unchanged. It appears on REST and both MCP
  transports only when a sinkhole or unresolvable host is detected.
- Fixed-origin provider clients now share one diagnostic transport; the
  diagnosis is best-effort and never masks or delays the real connection error.
- No change to provider contracts, OAuth scopes, tools, or the container images.
